...
const Doctors = () => {
  ...
  const {data:doctors, loading, error} = userFetchData(`${BASE_URL}/doctors?query=${debounceQuery}`);
  const handleSearch = () =>{
    setQuery(query.trim())
  ...
  return (
    <>
      ...
      <section>
        ...
        {loading  && <Loader />}
        {error  && <Error />}
        {!loading && !error && <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 md:grid-cols-3 gap-5 '>{
